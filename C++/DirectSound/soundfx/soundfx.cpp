...
VOID OnInitDialog( HWND hDlg )
{
    ...
    if( GetWindowsDirectory( strFile, MAX_PATH ) == 0 )
        return;
    wcscat_s( strFile, MAX_PATH, L"\\media\\ding.wav" );
                          ...
                          DWORD dwPrecision = 1 )
{
    ...
    {
        case 0:
            swprintf_s( tszstr, MAX_PATH, L"%.0f", val );
            SendMessage( GetDlgItem( hwndDlg, id + 1 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf_s( tszstr, MAX_PATH, L"%.0f", min );
            SendMessage( GetDlgItem( hwndDlg, id + 3 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf_s( tszstr, MAX_PATH, L"%.0f", max );
            SendMessage( GetDlgItem( hwndDlg, id + 4 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            break;
        case 1:
            swprintf_s( tszstr, MAX_PATH, L"%.1f", val );
            SendMessage( GetDlgItem( hwndDlg, id + 1 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf_s( tszstr, MAX_PATH, L"%.1f", min );
            SendMessage( GetDlgItem( hwndDlg, id + 3 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf_s( tszstr, MAX_PATH, L"%.1f", max );
            SendMessage( GetDlgItem( hwndDlg, id + 4 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            break;
        default:
        case 2:
            swprintf_s( tszstr, MAX_PATH, L"%.2f", val );
            SendMessage( GetDlgItem( hwndDlg, id + 1 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf_s( tszstr, MAX_PATH, L"%.2f", min );
            SendMessage( GetDlgItem( hwndDlg, id + 3 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf_s( tszstr, MAX_PATH, L"%.2f", max );
            SendMessage( GetDlgItem( hwndDlg, id + 4 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            break;
    }
...
VOID LoadParameterUI( HWND hwndDlg, DWORD dwFXType )
{
    ...
    swprintf_s( tszstr, MAX_PATH, L"Parameters for [ %s ]", g_tszFXNames[dwFXType] );
...
HRESULT ValidateWaveFile( HWND hDlg, TCHAR* strFileName )
{
    ...
    else // The load call succeeded
    {
        ...
        wcscpy_s( g_tszFilename, MAX_PATH, strFileName );
        return S_OK;
    }
}
