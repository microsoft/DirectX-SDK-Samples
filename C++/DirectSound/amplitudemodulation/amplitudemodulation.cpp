...
VOID OnOpenSoundFile( HWND hDlg )
{
    ...
    if( TRUE != GetOpenFileName( &ofn ) )
    {
        ...
        wcscpy_s( szCurName, MAX_PATH, g_strWaveFileName );
        if( SUCCEEDED( ValidateWaveFile( hDlg, szCurName ) ) )
        {
...
HRESULT ValidateWaveFile( HWND hDlg, TCHAR* strFileName )
{
    ...
    else // The load call succeeded
    {
        ...
        wcscpy_s( g_strWaveFileName, MAX_PATH, strFileName );
        return S_OK;
    }
}
...
VOID OnTimer( HWND hDlg )
{
    if( IsWindowEnabled( GetDlgItem( hDlg, IDC_STOP ) ) )
    {
        ...
        if( g_pSound && !g_pSound->IsSoundPlaying() )
        {
...
VOID OnEffectChanged( HWND hDlg )
{
    ...
    // Update UI
    TCHAR strBuffer[10];
    swprintf_s( strBuffer, 10, TEXT("%ld"), dsfxGargle.dwRateHz );
